...
function buildFallbackAnalysis(symbol: string, stockData: Record<string, unknown>) {
  ...
  if (roe !== null && roe >= 15) pros.push(`Healthy ROE at ${roe.toFixed(1)}%.`);
  if (debtToEquity !== null && debtToEquity <= 1) pros.push(`Manageable debt-to-equity at ${debtToEquity.toFixed(2)}.`);
  if (salesGrowth5yr !== null && salesGrowth5yr >= 10) pros.push(`Strong 5Y sales growth of ${salesGrowth5yr.toFixed(1)}%.`);
  if (dividendYield !== null && dividendYield >= 1.5) pros.push(`Dividend yield of ${dividendYield.toFixed(2)}% adds shareholder return.`);

  if (pe !== null && pe > 40) cons.push(`Valuation looks expensive with P/E at ${pe.toFixed(1)}.`);
  if (debtToEquity !== null && debtToEquity > 1.5) cons.push(`Leverage risk: debt-to-equity is ${debtToEquity.toFixed(2)}.`);
  if (profitVar5yr !== null && profitVar5yr < 0) cons.push(`5Y profit trend is weak at ${profitVar5yr.toFixed(1)}%.`);
  if (roe !== null && roe < 10) cons.push(`ROE at ${roe.toFixed(1)}% is below ideal profitability levels.`);
